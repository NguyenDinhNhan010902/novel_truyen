                        ...
                        <div className="relative group">
                            ...
                            <div className="absolute top-full left-0 w-[400px] bg-[#1c1c24] border border-white/10 rounded-xl shadow-2xl p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-2 group-hover:translate-y-0 z-50">
                                <div className="grid grid-cols-2 gap-2">
                                    {['Tiên Hiệp', 'Kiếm Hiệp', 'Ngôn Tình', 'Huyền Huyễn', 'Đô Thị', 'Khoa Huyễn', 'Võng Du', 'Dị Giới', 'Hệ Thống', 'Trọng Sinh', 'Lịch Sử', 'Quân Sự'].map(tag => (
                                        <Link key={tag} href={`/the-loai/${tag}`} className="block px-3 py-2 rounded-lg hover:bg-white/5 text-gray-400 hover:text-white transition-colors text-sm">
                                            {tag}
                                        </Link>
                                    ))}
                                </div>
