      ...
      <section className="relative overflow-hidden bg-[#13131a] pt-8 pb-12">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="relative rounded-2xl overflow-hidden bg-[#1c1c24] border border-white/5 shadow-2xl">
            <div className="grid md:grid-cols-[2fr_3fr] lg:grid-cols-[300px_1fr] gap-0">
              <Link href={`/truyen/${featuredNovel.slug}`} className="relative aspect-[2/3] md:aspect-auto group cursor-pointer">
                <img
              ...
              <div className="p-6 md:p-10 flex flex-col justify-center gap-4 relative">
                {/* Background texture */}
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  <p className="text-base md:text-lg text-gray-300 line-clamp-3 max-w-2xl font-light leading-relaxed">
                  ...
                  <div className="pt-6 flex gap-4">
                    <Link href={`/truyen/${featuredNovel.slug}`}>
                      <Button size="lg" className="rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 shadow-lg shadow-indigo-600/20">
                        Đọc Ngay
                      </Button>
                    </Link>
                    <Link href={`/truyen/${featuredNovel.slug}`}>
                      <Button variant="outline" size="lg" className="rounded-lg border-white/10 bg-white/5 hover:bg-white/10 text-white font-semibold flex items-center gap-2">
                        <Bookmark className="h-5 w-5" />
                        Đánh Dấu
                      </Button>
                    </Link>
                    {featuredNovel.audioUrl ? (
                      <Link href={featuredNovel.audioUrl} target="_blank" rel="noopener noreferrer">
                        <Button
                          size="lg"
                          className="rounded-lg bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-500 hover:to-pink-500 text-white font-bold shadow-lg shadow-rose-500/30 hover:shadow-rose-500/50 hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2"
                        >
                      ...
                      <Button
                        size="lg"
                        className="rounded-lg bg-gray-700 text-gray-400 cursor-not-allowed flex items-center gap-2 opacity-50"
                        disabled
                      >
                        <Headphones className="h-5 w-5" />
                        Nghe Truyện
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[#1c1c24] rounded-xl border border-white/5 p-6 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {['Tiên Hiệp', 'Kiếm Hiệp', 'Huyền Huyễn', 'Đô Thị', 'Võng Du', 'Lịch Sử'].map(tag => (
                <Link key={tag} href={`/the-loai/${tag}`} className="px-3 py-1.5 bg-[#272732] rounded-lg text-xs text-gray-400 hover:text-white hover:bg-indigo-600 transition-all cursor-pointer">
                  {tag}
                </Link>
              ))}
            </div>
