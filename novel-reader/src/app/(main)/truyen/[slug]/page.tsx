            ...
            <div className="container mx-auto px-2 md:px-4 py-4 md:py-8">
                ...
                <div className="bg-[#1c1c24] rounded-2xl border border-white/5 p-4 md:p-10 shadow-xl relative overflow-hidden">
                    ...
                    <div className="grid md:grid-cols-[260px_1fr] gap-6 md:gap-12 relative z-10">
                        {/* Cover - Book Style */}
                        <div className="mx-auto md:mx-0 w-[160px] md:w-full aspect-[2/3] rounded-lg shadow-2xl overflow-hidden border-2 border-yellow-600/30 relative group">
                        ...
                        <div className="space-y-4 md:space-y-6 text-center md:text-left">
                            ...
                            <div className="pt-2 md:pt-4 grid grid-cols-2 md:flex md:flex-wrap gap-3 md:gap-4">
                                <Link href={firstChapter ? `/doc/${novel.slug}/${firstChapter.id}` : '#'} className="col-span-2 md:col-span-1 md:w-auto">
                                    <Button size="lg" className="w-full md:w-auto h-10 md:h-12 px-8 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm md:text-base shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-2" disabled={!firstChapter}>
                                        <BookOpen className="h-4 w-4 md:h-5 md:w-5 flex-shrink-0" />
                                        <span className="whitespace-nowrap">Đọc Từ Đầu</span>
                                    </Button>
                                </Link>
                                <Button variant="outline" size="lg" className="w-full md:w-auto h-10 md:h-12 px-6 rounded-xl border-white/10 bg-white/5 hover:bg-white/10 text-white font-semibold flex items-center justify-center gap-2">
                                    <Bookmark className="h-4 w-4 md:h-5 md:w-5 flex-shrink-0" />
                                ...
                                {novel.audioUrl ? (
                                    <Link href={novel.audioUrl} target="_blank" rel="noopener noreferrer" className="block w-full md:w-auto">
                                        <Button
                                            size="lg"
                                            className="w-full md:w-auto h-10 md:h-12 px-6 rounded-xl bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-500 hover:to-pink-500 text-white font-bold shadow-lg shadow-rose-500/30 hover:shadow-rose-500/50 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2"
                                        >
                                            <Headphones className="h-4 w-4 md:h-5 md:w-5 flex-shrink-0" />
                                    ...
                                    <Button
                                        size="lg"
                                        className="w-full md:w-auto h-10 md:h-12 px-6 rounded-xl bg-gray-700 text-gray-400 cursor-not-allowed flex items-center justify-center gap-2 opacity-50"
                                        disabled
                                    >
                                        <Headphones className="h-4 w-4 md:h-5 md:w-5 flex-shrink-0" />
                                        <span className="whitespace-nowrap">Nghe Truyện</span>
